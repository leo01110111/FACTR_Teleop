    ...
    def _validate_response(self, response: dict) -> tuple[Dict[str, np.ndarray], str, str, float, float]:
        ...
        for side in self._active_sides:
            ...
            if side in self._latest_state:
                ...
                max_distance = self._max_safe_target_distance_rad
                ...
                # Clamp here too so a delayed q_safe cannot create a servoJ jump.
                # Dropping the target entirely causes the follower to hold a stale
                # command and can make every later response fail the same check.
                delta = q_safe - q_current
                if np.max(np.abs(delta)) > max_distance:
                    q_safe = q_current + np.clip(delta, -max_distance, max_distance)
        ...
        if clipped_sides:
            suffix = "bridge_clipped_to_current:" + ",".join(clipped_sides)
